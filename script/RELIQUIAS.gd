...
func _ready():
	...
	if current == "Codó":
		...
		$Sprite.texture = load("res://tudo/imagens/reliquia/reliquia_1.jpeg" )
	elif current == "Alcântara":
		$Label.text = "PARABÉNS,\nVOCÊ ENCONTROU TODAS AS RELÍQUIAS!"
		$botao1/Label.text =" CONTINUAR"
		$Sprite.texture = load( "res://tudo/imagens/reliquia/reliquia1.jpeg")
	else:
		if num2 == "-1":
			$Sprite.texture = load("res://tudo/imagens/sino_azul/sino0azul.jpg" )
